...
# Run from repo root: npm run deploy:local-clean
...
spacetime publish -c --no-config -p . mammoth-local -y
...
Write-Host "[SUCCESS] Clean local deployment complete! Database: mammoth-local" -ForegroundColor Green
