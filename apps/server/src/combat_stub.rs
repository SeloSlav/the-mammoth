...
/// Selected hotbar item `def_id` when the combat rail points at a real stack; otherwise `None`.
...
pub fn stub_melee_damage_for_active_loadout(ctx: &ReducerContext, attacker: Identity) -> f32 {
    let Some(def_id) = active_hotbar_weapon_def_id(ctx, attacker) else {
        return STUB_FIST_BASE_DAMAGE;
    };
    let w = stub_melee_damage_for_def_id(&def_id);
    if w > 0.0 {
        w
    } else {
