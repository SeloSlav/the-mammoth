 ...
 *
 * Toggle: URL `?apartmentEdit`, or `localStorage.setItem('mammothApartmentLayoutAuthoring','1')`, then **`F9`**.
...
} from "./fpApartmentLayoutPieces.js";
import { patchFpTransformControlsPointerForCaptureCompat } from "./fpTransformControlsPointerPatch.js";
...
export function apartmentLayoutUiGateOpen(): boolean {
  try {
    ...
    if (window.localStorage.getItem("mammothApartmentLayoutAuthoring") === "1") return true;
    return new URLSearchParams(window.location.search).has("apartmentEdit");
  } catch {
    return false;
  }
}

function ownedInteriorUnit(conn: DbConnection): ApartmentUnit | null {
...
export function normalizeApartmentDecorModelRelPath(raw: string): string | null {
  let t = raw.trim().replace(/^\/+/u, "");
  if (t.includes("..")) return null;
  const lower = t.toLowerCase();
  if (!lower.endsWith(".glb")) return null;
  const full = t.startsWith("static/models/") ? t : `static/models/${t}`;
  if (!full.startsWith("static/models/")) return null;
  if (full.length < 14 || full.length > 210) return null;
  const okChars = /^[a-zA-Z0-9/._-]+$/u.test(full);
  if (!okChars) return null;
  return full;
}
...
export function mountFpApartmentLayoutAuthoring(
  ...
  let panelVisible =
    apartmentLayoutUiGateOpen() &&
    typeof window !== "undefined" &&
    new URLSearchParams(window.location.search).has("apartmentEdit");
  ...
  hint.innerHTML =
    "<b>F9</b> toggles · crosshair-ray picks wardrobe / locker / bed · décor uses meshes. Feet must be inside your apartment. Imports only <code>/static/models/...</code>.glb (add file under client <code>public/static/models/</code>).";
  ...
  decorInput.type = "text";
  decorInput.placeholder = "objects/my_prop.glb (under public/static/models/)";
  decorInput.autocomplete = "off";
  decorInput.style.boxSizing = "border-box";
  decorInput.style.width = "100%";
  decorInput.style.marginBottom = "6px";
  decorInput.style.padding = "5px";
  decorInput.style.borderRadius = "4px";

  const addDecorBtn = document.createElement("button");
  addDecorBtn.type = "button";
  addDecorBtn.textContent = "Spawn décor at feet";
  ...
  shell.append(title, hint, pieceSelect, mkModeRow(), decorInput, addDecorBtn, delDecorBtn);
  ...
  function syncStagingFromReplica(): void {
    ...
    const unit = ownedInteriorUnit(opts.conn);
  ...
  addDecorBtn.addEventListener("click", () => {
    const unit = ownedInteriorUnit(opts.conn);
    if (!unit) return;
    const path = normalizeApartmentDecorModelRelPath(decorInput.value.trim());
    if (!path) {
      window.alert("Invalid path — use static/models/**.glb (see placeholder).");
      return;
    }
