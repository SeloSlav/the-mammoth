...
function isCombatSimNpcRow(row: WorldNpc): boolean {
  return row.sessionKey.startsWith("combat_sim:");
}

function isNpcRowVisible(row: WorldNpc): boolean {
  if (row.state === NPC_STATE_DEAD) return false;
  if (row.health <= 0 && !isCombatSimNpcRow(row)) return false;
  return true;
}
...
function snapshotFromRow(row: WorldNpc, observedTimeMs: number): ReplicatedNpcSnapshot | null {
  ...
  if (!isNpcRowVisible(row)) return null;
  return {
    npcId: row.npcId,
    archetype,
...
export function createFpNpcSession(opts: {
  worldParent: import("three").Object3D;
  fxScene: import("three").Scene;
  conn: DbConnection;
