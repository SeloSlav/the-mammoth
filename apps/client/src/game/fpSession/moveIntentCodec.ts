...
/** Inverse of {@link encodeMoveIntentBits} — mutates `out`, allocates nothing. */
export function decodeMoveIntentBitsInto(bits: number, out: FpLocomotionInput): void {
  out.forward = (bits & BIT_FORWARD) !== 0;
  out.backward = (bits & BIT_BACK) !== 0;
  out.left = (bits & BIT_LEFT) !== 0;
  out.right = (bits & BIT_RIGHT) !== 0;
  out.sprint = (bits & BIT_SPRINT) !== 0;
  out.crouch = (bits & BIT_CROUCH) !== 0;
  out.jumpHeld = (bits & BIT_JUMP_HELD) !== 0;
}
