...
import { APARTMENT_FISH_TANK_MODEL_PATH } from "./apartmentFishTankVisual.js";
...
describe("apartmentProceduralDecorCatalog", () => {
  it("includes procedural paths in merged manifest lists", () => {
    ...
    expect(merged).toContain(APARTMENT_FISH_TANK_MODEL_PATH);
  });
  ...
  it("recognizes procedural decor paths", () => {
    ...
    expect(isProceduralApartmentDecorModelPath(APARTMENT_FISH_TANK_MODEL_PATH)).toBe(true);
  ...
  it("builds procedural visuals for catalog paths", () => {
    expect(buildProceduralApartmentDecorVisual(APARTMENT_FISH_TANK_MODEL_PATH)?.name).toBe(
      "apartment_fish_tank",
    );
