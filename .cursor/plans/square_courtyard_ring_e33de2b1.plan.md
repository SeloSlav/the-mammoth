...
overview: Rewrite the Mamutica floor generator to emit a square courtyard ring (4 perimeter corridors, double-loaded, compact 80 m outer footprint) and regenerate all derived content. No gameplay code changes needed — the adjacency, door-face, and collision pipelines are already fully axis-agnostic.
todos:
  - id: rewrite-gen
    ...
    status: pending
  - id: regen-floor-json
    content: Run pnpm content:gen-mamutica-floor and eyeball both floor JSONs (~48 units/floor, 4 corridor segments, 4 stairs, 4 elevators)
    status: pending
  - id: regen-doors
  ...
  - id: regen-walk
    ...
    status: pending
  - id: metadata-touchup
    content: Update mammoth.json gameplay_numbering_note to describe the ring layout
    status: pending
  - id: smoke-check
    content: Run scripts/check-apartment-door-passthrough.ts and confirm all faces (including newly rotated N/S bars) pass
    status: pending
isProject: false
---
...
### 3. Touch-up [content/building/mammoth.json](content/building/mammoth.json) metadata
...
- No editor/UI code changes, no runtime code changes. Only authoring JSON + the generator script.
...
- **Spawn position**: After regen, player spawns inside the courtyard (x=z=0). They need to walk through the corridor to reach a unit. If you want spawn in the south lobby instead, that is a one-line `recommended_spawn_xz_m` tweak and I can include it now on request.
