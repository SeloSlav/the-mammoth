...
- **`fpViewmodel`** (optional, FP only): hand GLB pose, grip socket on the hand, and crowbar mesh scale after max-edge normalize. The editor / `?fpAuthor` dev overlay exports `firstPersonMerge` — merge `mount` + `fpViewmodel` into `firstPerson` and commit; `LocalFirstPersonPresenter` reads this at load (no TS merge step).
